...
export default async function Home() {
  const { testimonials, c } = await getData()
      ...
      <section className="hero">
        ...
        <div className="hero__content container">
          <h1 className="display-lg text-white">
            {c.hero_title?.length <= 0 || !c.hero_title ? "L'Art de la Table à la Française" : c.hero_title}
          </h1>
          <p className="hero__subtitle text-white">
            {c.hero_subtitle?.length <= 0 || !c.hero_subtitle ? "Au service des hôteliers et restaurateurs d'exception" : c.hero_subtitle}
          </p>
            ...
            testimonials.map((t) => (
              <div key={t.id} className="testimonial__card">
                ...
                <div className="author">
                  ...
                  <div>
                    <p className="author__name">{t.author}</p>
                    {t.role && <p className="author__role">{t.role}</p>}
                  </div>
                </div>
              </div>
            ))
